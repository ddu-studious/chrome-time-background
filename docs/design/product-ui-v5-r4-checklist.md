...
> 基线：18 个业务、112 个页面。只有状态为 `R4` 且有真实 Chrome 视觉与交互证据，才视为该页完成；`R4-partial` 不计完成。
>
> 当前：50/112 页达到 `R4`，1 页为 `R4-partial`。
...
| ✅ | 应用启动台 | `launchpad/all-apps` | 全部应用 | R4 | 真实扩展已验收打开/关闭；Chrome 本地真实页面进一步确认 20 应用、7 分类、最近使用、dialog/inert、Tab 闭环和精确焦点回归 |
...
| ⬜ | 设置 | `settings/about-diagnostics` | 关于与诊断 | R3 | Chrome 本地真实页面已验收性能/关于/诊断三段、112 页注册、localStorage 降级、外部网络未访问及切页回页首；待扩展上下文、版本和 storage 复核 |
