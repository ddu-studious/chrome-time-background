...
> 2026-05-17 更新：标记各项借鉴能力的实现状态

### 6.1 短期可落地（Phase 2 融合）

| 借鉴来源 | 具体行动 | 优先级 | 状态 | 备注 |
|---------|---------|-------|------|------|
| Deep Agents | 引入 `write_todos` 式进度追踪到 Agent 卡片 | P0 | **未实现** | 需在前端 Agent 卡片中集成任务进度条 |
| gstack | Agent 模板升级为"角色 Skill"（含行为模式、禁止列表） | P0 | **已实现** | `enterprise-roles.ts` 已定义 10 个角色含行为/约束 |
| Hermes | Agent 间结果引用 → 用 `@agent-name` 引用输出 | P0 | **未实现** | 讨论中角色可引用前序发言，但无 `@` 引用语法 |
| browser-harness | Agent 通过扩展 API 控制当前页面（测试/截图） | P1 | **部分实现** | `browser-automation.ts` 存在但 CDP 集成待完善 |
| Superpowers | 工作流强制验证步骤（Human-in-the-Loop） | P1 | **已实现** | `agent-pool.ts` 中 Approval Gate 机制已就绪 |

### 6.2 中期架构演进（Phase 3-4）

| 借鉴来源 | 具体行动 | 状态 | 备注 |
|---------|---------|------|------|
| autoresearch | "实验-评估-迭代"闭环 → Agent 自动运行代码并根据结果决策 | **未实现** | 需集成代码执行沙箱 |
| Hermes | Skill 自学习 → 用户常用工作流自动保存为模板 | **未实现** | 需实现工作流录制 + 模板保存 |
| Deep Agents | 文件系统工作记忆 → Agent 间通过虚拟文件系统共享数据 | **未实现** | 当前 Agent 间通过讨论消息传递 |
| gstack | 认知齿轮切换 → 工作流阶段自动切换 Agent 行为模式 | **部分实现** | Phase 分阶段执行已有，但角色行为不会动态切换 |

### 6.3 核心能力缺失清单

| # | 缺失能力 | 严重度 | 描述 |
|---|---------|-------|------|
| 1 | **模型选择机制** | 高 | 所有角色硬编码 `claude-sonnet-4-6`，无法按角色/任务智能分配模型 |
| 2 | **用户自定义角色模型** | 高 | 用户无法为每个角色指定不同的 LLM 模型 |
| 3 | **自动模型路由** | 中 | 缺少按任务复杂度/角色类型自动选择最佳模型的策略 |
| 4 | **Agent 进度追踪** | 中 | 无 TodoWrite 式任务分解与进度可视化 |
| 5 | **Agent 间引用** | 中 | 无 `@agent-name` 引用机制 |
| 6 | **代码执行沙箱** | 中 | Agent 无法运行代码并根据结果迭代 |
| 7 | **Skill 自学习** | 低 | 无法从历史工作流中自动提取可复用模板 |
| 8 | **跨会话记忆** | 低 | FTS5 全文搜索已有，但缺少 LLM 摘要式长期记忆 |
| 9 | **一键部署流程** | 低 | 缺少自动化 PR → CI → Deploy 流程 |
| 10 | **多模型并行** | 低 | 无法同时用多个模型处理同一任务并取最优结果 |
