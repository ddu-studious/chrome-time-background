...
test('设置中心映射八个设计页面并复用现有设置表单', () => {
  const html = read('settings.html');
  const source = read('js/settings-v5.js');
  for (const page of ['appearance', 'homepage', 'dock', 'data-privacy', 'integrations', 'ai-control', 'shortcuts', 'about-diagnostics']) {
  ...
  assert.match(html, /js\/settings-v5\.js\?v=7/);
  assert.match(html, /css\/settings-v5\.css\?v=7/);
});
